...
const UserManagement = () => {
  ...
  const getRoleBadge = (roles) => {
    if (roles.includes("ROLE_ADMIN")) {
      return (
        <span className="bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 w-fit">
    ...
    if (roles.includes("ROLE_CREATOR")) {
      return (
        <span className="bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 w-fit">
      ...
      {pageInfo.totalPages > 1 && (
        <div className="flex items-center justify-between mt-4 flex-wrap gap-4">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Hiển thị {from} - {to} trên {pageInfo.totalElements} người dùng
          </p>
          <Pagination
