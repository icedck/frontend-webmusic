...
const CreateSongAdmin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (singersRes.success)
          setSingerOptions(
            Array.isArray(singersRes.data)
              ? singersRes.data.map((s) => ({ value: s.id, label: s.name }))
              : []
          );
        if (tagsRes.success)
          setTagOptions(
            Array.isArray(tagsRes.data)
              ? tagsRes.data.map((t) => ({ value: t.id, label: t.name }))
              : []
          );
      } catch (err) {
  ...
  const handleSingersChange = (selectedOptions) => {
    ...
    const existingNewData = newSingersData.filter((ns) =>
      newOnes.some((n) => n.value === ns.value)
    );
  ...
  const updateNewSingerData = (singerLabel, key, value) => {
    setNewSingersData((prev) =>
      prev.map((s) => (s.label === singerLabel ? { ...s, [key]: value } : s))
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSingers.length === 0)
      return toast.error("Vui lòng chọn hoặc tạo ít nhất một ca sĩ.");
    if (!audioFile) return toast.error("Vui lòng chọn file audio.");
    setLoading(true);

    const singerIds = selectedSingers
      .filter((s) => !s.__isNew__)
      .map((s) => s.value);
    ...
    data.append(
      "songRequest",
      new Blob([JSON.stringify(songRequest)], { type: "application/json" })
    );
    ...
    option: (base, { isFocused, isSelected }) => ({
      ...
      backgroundColor: isSelected
        ...
        ? isDarkMode
          ...
          : "#e5e7eb"
        : "transparent",
      color: isSelected ? "white" : isDarkMode ? "#d1d5db" : "#111827",
    }),
    multiValue: (base) => ({
      ...base,
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className={`mt-2 ${currentTheme.textSecondary}`}>
          Điền thông tin chi tiết cho bài hát mới.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          ...
          <textarea
            ...
            className={`w-full p-2 border rounded-lg ${currentTheme.bg} ${currentTheme.border} focus:border-cyan-500 focus:ring-cyan-500`}
          />
          <div>
            <label
              htmlFor="lyrics"
            ...
            <textarea
              ...
              className={`w-full p-2 border rounded-lg ${currentTheme.bg} ${currentTheme.border} focus:border-cyan-500 focus:ring-cyan-500 font-mono`}
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <CreatableSelect
                ...
                placeholder="Chọn hoặc nhập tên ca sĩ mới..."
                formatCreateLabel={(inputValue) =>
                  `Thêm mới ca sĩ "${inputValue}"`
                }
                isLoading={isFetchingOptions}
                isDisabled={isFetchingOptions}
                styles={selectStyles}
                noOptionsMessage={() => "Không tìm thấy ca sĩ"}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-700">
              ...
              {newSingersData.map((singer) => (
                <NewSingerInput
                  key={singer.value}
                  singer={singer}
                  onUpdate={updateNewSingerData}
                  onRemove={removeNewSinger}
                />
              ))}
            </div>
          )}
