...
const mainNavItems = [
  ...
  { href: '/library', icon: Library, label: 'Thư viện', roles: ['user', 'creator', 'admin'] },
  { href: '/charts', icon: BarChart3, label: 'Bảng xếp hạng', roles: ['all'] },
];

const creatorNavItems = [
  { href: '/creator/my-submissions', icon: SongIcon, label: 'Bài hát của tôi', roles: ['creator'] },
];

const adminNavItems = [
  { href: '/admin/users', icon: AdminIcon, label: 'Quản lý người dùng', roles: ['admin'] },
...
const AppHeader = ({ onOpenPalette, isPlayerVisible, onConfirmLogout }) => {
  ...
  const getVisibleNavItems = () => {
    let allItems = [
      ...mainNavItems,
      { type: 'button', action: onOpenPalette, icon: Search, label: 'Tìm kiếm (⌘K)', roles: ['all'] }
    ];
    if (user) {
      if (isCreator()) allItems.push(...creatorNavItems);
      if (isAdmin()) allItems.push(...adminNavItems);
    } else {
      allItems = allItems.filter(item => item.roles.includes('all'));
    }
    return [...new Map(allItems.map(item => [item.label, item])).values()];
  };
