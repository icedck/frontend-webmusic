        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            {top3.map((entry, index) => {
                ...
                return (
                    <div 
                        key={entry.song.id}
                        className={`relative p-6 rounded-2xl border-2 transition-all duration-300 ${colors.bg} ${colors.border} hover:scale-105 hover:shadow-lg group`}
                    >
                        {/* Rank Number */}
                        <div className={`absolute top-4 left-4 w-8 h-8 rounded-full flex items-center justify-center text-lg font-black text-white ${colors.accent}`}>
                            {index + 1}
                        </div>
                        ...
                        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                            <Button
                                ...
                                className="w-10 h-10 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-full text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 shadow-lg"
                            >
                                {isCurrentlyPlaying ? (
                                    <Pause size={18} />
                                ) : (
                        ...
                        <div className="relative mx-auto mb-4" style={{ width: '120px', height: '120px' }}>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black/40 rounded-xl">
                                <Button
                                    size="icon"
                                    onClick={() => onPlaySong(entry.song)}
                                    className="w-12 h-12 bg-white/20 hover:bg-white/40 backdrop-blur-sm rounded-full text-white border-none"
                                >
                                    {isCurrentlyPlaying ? (
                                        <Pause size={24} />
                                    ) : (
                        ...
                        <div className="text-center">
                            <Link 
                                ...
                                className={`block font-bold text-lg mb-2 truncate hover:underline transition-colors ${
                                    isCurrentlyPlaying 
                            ...
                            <div className={`text-sm mb-3 truncate ${colors.text}`} title={entry.song.singers?.map(s => s.name).join(', ')}>
                                {entry.song.singers?.map(s => s.name).join(', ') || 'Unknown Artist'}
                            </div>

                            {/* Percentage */}
                            <div className={`text-3xl font-black ${colors.number} mb-2`}>
                                {percentages[index]}
                            </div>
