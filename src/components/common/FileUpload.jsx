...
const FileUpload = ({ label, accept, onFileChange, previewType = 'icon' }) => {
    ...
    const renderPreview = () => {
        ...
        return file?.type.startsWith('image/') ? <ImageIcon className="w-10 h-10 text-slate-500" /> : <File className="w-10 h-10 text-slate-500" />;
    };
        ...
        <div>
            ...
            {!file ? (
                ...
                <div className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4">
                    ...
                    <button type="button" onClick={handleRemoveFile} className="p-2 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700">
                        <X className="w-5 h-5" />
                    </button>
                </div>
            )}
